CREATE MATERIALIZED VIEW mv_bike_availability_by_neighborhood_10min AS
WITH station_neighborhood AS (
    SELECT
        s.station_id,
        l.neighbourhood_name,
        l.neighbourhood_code
    FROM
        dim_station s
    JOIN
...
station_metrics AS (
    ...
    JOIN
        station_neighborhood sn ON ss.station_id = sn.station_id
    WHERE
        m.year = 2019 AND
        m.month = 6 AND
...
FROM
    station_metrics
GROUP BY
    neighbourhood_name, neighbourhood_code, date_value, hour, ten_min_datetime
ORDER BY
    ten_min_datetime, neighbourhood_name;

-- Create indexes for performance
CREATE INDEX idx_mv_bike_10min_neighborhood_datetime
ON mv_bike_availability_by_neighborhood_10min(ten_min_datetime);

CREATE INDEX idx_mv_bike_10min_neighborhood
ON mv_bike_availability_by_neighborhood_10min(neighbourhood_code); 